 ...
 *
 *
 *  Given a hypergraph, the main idea is to find the noisy hyperedges and to transform them (e.g. median filter, mean filter, ...).
 *
 */

// the template argument is a pixel-based image hypergraph
...
private:
    ...
    /* method : openTest
     * params :
     *      _hyper : a pixel-based image hypergraph
     *      _ng    : contains some hyperedge indices of  _hyper
     *      _x     : the index of an element of _ng
     *
     * output : boolean = True if a neighbor of the element _ng indexed by _x has its cardinal equals to one,
     *         False else
     */
    static bool openTest(const HyperRep &_hyper, HyperEdge * _ng,   long _x);

    /* method : noisyHyperEdges 
     * params : _hyper = a pixel-based image hypergraph
     * output : All noisy hyperedges of _hyper, their indices are stored in a set (here Set<long> which is HyperEdge)
     */
...
public:
     ...
     /* method : hyperDenoise
      * params : 
           - _hyper  = is a pixel based image hypergraph representation
	   - _output = the output image denoised
      */
     static void hyperDenoise(HyperRep & _hyper, IplImage * _output);
     
     // this fonction compute the psnr given two images
...
void HyperDenoise<HyperRep>::hyperDenoise(HyperRep & _hyper, IplImage * _output)
{
    ...
    for(unsigned int i = 0; i < nh->size(); i++){
      ...
      ng = _hyper.neighborHyperEdgeSets( edge);
