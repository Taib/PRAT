...
class Slic {
    ...
    public:
        ...
        /* Get/Set functions */
        inline vec2di getClusters(){return clusters;}
        inline vec2dd getCenters(){return centers;}
        inline vec2dd getDistances(){return distances;}
        inline vector<int> getCenterCounts(){return center_counts;}
};
...
void Slic::display_contours(IplImage *image, CvScalar colour) {
    ...
    for (int i = 0; i < image->width; i++) {
        for (int j = 0; j < image->height; j++) {
            ...
            /* Add the pixel to the
             * contour list if desired. */
            if (nr_p >= 2) {
